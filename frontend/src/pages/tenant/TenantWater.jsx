...
export default function TenantWater() {
  ...
  const fetchInitialData = async () => {
    try {
      setLoading(true);
      const resHistory = await api.get('/tenant-portal/water/history?months=36');
      ...
      setError(err.message || 'Không thể tải chỉ số nước.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const resolveReadingForPeriod = async (m, y) => {
    ...
    try {
      ...
      const res = await api.get(`/tenant-portal/water/current?month=${m}&year=${y}`);
  ...
  if (loading) {
    return <MeterDetailSkeleton type="water" />;
  }
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
              <h2 style={{ fontSize: '20px', fontWeight: '800', color: '#0f172a', margin: 0 }}>
                Tiền Nước - Tháng {selectedMonth}/{selectedYear}
              </h2>
        ...
        <div className="grid-responsive-2">
          ...
          <div style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#0284c7' }}>
                  <Droplet size={28} />
                  <div>
                    <span style={{ fontSize: '18px', fontWeight: '800', display: 'block' }}>Chi tiết tiêu thụ nước</span>
                  </div>
                </div>
                <span style={{
                  display: 'inline-flex',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', fontSize: '17px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#475569' }}>
                  <span>Chỉ số cũ:</span>
                  <span style={{ fontWeight: '700', color: '#0f172a' }}>{formatNumber(reading?.previous_value || 0)} m³</span>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#475569' }}>
                  <span>Chỉ số mới:</span>
                  <span style={{ fontWeight: '700', color: '#0f172a' }}>{formatNumber(reading?.current_value || 0)} m³</span>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#475569', borderTop: '1px dashed #e2e8f0', paddingTop: '10px' }}>
                  <span>Số khối nước đã dùng:</span>
                  <span style={{ fontWeight: '800', color: '#0284c7', fontSize: '18px' }}>
                    {formatNumber(reading?.consumption || 0)} m³
                  </span>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#475569' }}>
                  <span>Đơn giá nước:</span>
                  <span style={{ fontWeight: '700', color: '#0f172a' }}>{formatCurrency(reading?.unit_price || 12000)} / m³</span>
                </div>
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: '14px', color: '#0369a1', fontWeight: '700', textTransform: 'uppercase' }}>
                THÀNH TIỀN NƯỚC THÁNG {selectedMonth}/{selectedYear}
              </div>
              <div style={{ fontSize: '30px', fontWeight: '900', color: '#0284c7', marginTop: '4px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px', marginBottom: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <h3 style={{ fontSize: '17px', fontWeight: '700', color: '#0f172a', margin: 0 }}>
                    Hình ảnh đồng hồ nước chụp thực tế
                  </h3>
                </div>
              </div>
              ...
              {reading?.image_url ? (
                ...
                >
                  <img
                    ...
                    alt={`Đồng hồ nước Tháng ${selectedMonth}/${selectedYear}`}
        ...
        <div style={{
          ...
          <div>
            <h3 style={{ fontSize: '18px', fontWeight: '800', color: '#0f172a', margin: 0 }}>
              Chưa có dữ liệu tiền nước Tháng {selectedMonth}/{selectedYear}
            </h3>
            <p style={{ fontSize: '14px', color: '#64748b', maxWidth: '440px', margin: '8px auto 0' }}>
              Chủ nhà chưa chốt hoặc chưa ghi nhận chỉ số đồng hồ nước cho kỳ này. Bạn có thể chọn kỳ khác hoặc bấm nút bên dưới để xem kỳ gần nhất.
            </p>
          </div>
      ...
      {isZoomed && reading?.image_url && (
        <div className="modal-backdrop" onClick={() => setIsZoomed(false)}>
          <div style={{ position: 'relative', maxWidth: '90vw', maxHeight: '90vh' }} onClick={(e) => e.stopPropagation()}>
            ...
            <img
              src={reading.image_url}
              alt={`Chi tiết đồng hồ nước Tháng ${selectedMonth}/${selectedYear}`}
